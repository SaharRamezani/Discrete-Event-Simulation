...
#The workloads.py file provides tools for generating workloads using traces like the Mustang dataset.
...
def parse_mustang(path=None):
    ...
    if not os.path.exists(path):
        with urlopen(MUSTANG_URL,context=ssl_context) as url, NamedTemporaryFile(delete=False) as tmp:
...
if __name__ == '__main__':  # sanity check
    lamda = 0.7
    normalize_trace(parse_mustang(), lamda)
