...
class Backup(Simulation):
    ...
    def check_redundancy(self):
        """
        Checks redundancy. 
        If redundancy is below the threshold, we increase `n_active` on certain nodes.
        """
...
class RedundancyCheckEvent(Event):
    """Event to check system redundancy and possibly increase n_active."""
    def process(self, sim: Backup):
        sim.check_redundancy()  # calls the method that decides if we need to raise n_active
        # schedule the next redundancy check for the next interval
        sim.schedule(parse_timespan("1 week"), RedundancyCheckEvent())
...
class Node:
    ...
    def __post_init__(self):
        ...
        # if not specified, default n_active to n (i.e., use all blocks)
        if self.n_active == -1 or self.n_active > self.n or self.n_active is None:
        ...
        self.local_blocks: List[bool] = [True] * self.n_active
        ...
        self.free_space: int = self.storage_size - self.block_size * self.n_active

        # Instead of storing space for all n blocks, store space only for n_active blocks
        if self.n_active != self.n: # extension is enabled
            used_blocks_space = self.block_size * self.n_active
            self.free_space: int = self.storage_size - used_blocks_space
            self.local_blocks = ([True] * self.n_active) + ([False] * (self.n - self.n_active))
    ...
    def find_block_to_back_up(self):
        ...
        for block_id in range(self.n_active):
            assert self.n_active <= self.n, self.n_active
    ...
    def schedule_next_download(self, sim: Backup):
        ...
        # sim.log_info(f"schedule next download on {self}")
        ...
        for peer in sim.nodes:
            if (peer is not self and peer.online and peer.current_upload is None
                    and peer not in self.remote_blocks_held and self.free_space >= peer.block_size):
    ...
    def set_n_active(self, new_n_active: int):
        """
        1) Increase n_active up to new_n_active (<= n).
        2) Ensures there is enough space before increasing `n_active`.
        """

        if new_n_active <= self.n_active:
            return  # Do nothing if it's not an increase or not enough local blocks for new redundant ones

        additional = new_n_active - self.n_active
        space_needed = self.block_size * additional
        # Amount of space it has after recovery (if needed) - We want to prevent free_space < 0
        free_space = self.storage_size - self.block_size * self.n_active - len(self.remote_blocks_held) * self.block_size

        assert free_space >= 0 # It cannot be less than 0

        # Prevent free_space < 0 after recovery
        deleted_blocks_owners = []
        while free_space < space_needed and self.remote_blocks_held:
            # Remove some stored blocks for other nodes to free space
            owner, block_id = self.remote_blocks_held.popitem()
            owner.backed_up_blocks[block_id] = None
            deleted_blocks_owners.append(owner)

            free_space += self.block_size
            self.free_space += self.block_size

        # If we still don't have enough space, reduce the increase
        space_we_can_afford = free_space - space_needed

        if space_we_can_afford < 0:
            new_blocks_we_can_afford = free_space // self.block_size
            new_n_active = self.n_active + new_blocks_we_can_afford

        self.n_active = new_n_active

        if sum(self.local_blocks) >= self.k:
            for b in range(new_n_active - new_blocks_we_can_afford, new_n_active):
                self.local_blocks[b] = True
            self.free_space -= self.block_size * new_blocks_we_can_afford

            if self.online:
                self.schedule_next_upload(self)

        for owner in deleted_blocks_owners:
            if owner.online:
                owner.schedule_next_upload(owner)
...
class Fail(Disconnection):
    ...
    def process(self, sim: Backup):
        sim.log_info(f"fails {self.node}")
        self.disconnect()
        node = self.node
        node.failed = True
        ...
        node.free_space = node.storage_size - node.block_size * node.n_active
...
class BlockRestoreComplete(TransferComplete):
    def update_block_state(self, sim:Backup):
        ...
        if sum(owner.local_blocks) >= owner.k:
            owner.local_blocks = [True] * owner.n_active 
            owner.failed = False
...
def main():
    ...
    parser.add_argument("--n-active", type=int, default=-1, 
                        help="Number of active blocks to use out of n (overrides config).")
    parser.add_argument("--tolerance", type=int, default=-1, 
                        help="Difference between blocks that we have and blocks that we need. If it reaches below  this value, we add redundancy.")
    ...
    for node_class in config.sections():
        ...
        cfg = []
        for name, parse in parsing_functions:
            if name == 'n_active' and args.n_active is not None:
                cfg.append(args.n_active)  # Use command-line value
            elif name == 'tolerance' and args.tolerance is not None:
                cfg.append(args.tolerance)  # Use command-line value
            else:
                cfg.append(parse(class_config[name]))
    ...
    if args.n_active is not None:
        # schedule the first redundancy check in 1 week
        sim.schedule(parse_timespan("1 week"), RedundancyCheckEvent())
